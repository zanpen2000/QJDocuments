...
> 2. 现在有些接口因为后端的反射写法，一个接口多个实现类的方式，需要前端在post方法的body中传tactics字段；现要求后端在写反射方法的时候，将需要前端传的反射字段放在url地址上，不要放在body里面加字段，具体示例如下：

```demo
错误示例：/base/authorization/account/accountDisable 账号停用  
        /base/authorization/account/accountEnable 账号启用
正确示例：  /base/authorization/account/{tactics} 账号管理
也就是说，区分这个接口的作用，在url中，由前端传字段，来实现后端的反射方法。
在看接下来的示例：
错误示例：/base/authorization/foreign/authen/accountInfoByAccount/{account} 根据账号获取账号信息
        /base/authorization/foreign/authen/accountInfoByPhone/{phone} 按手机号查询账号信息
这种情况可以使用反射，那么就需要前端传辨别字段，也就是说，后端统一一个接口：/base/authorization/foreign/authen/{tactics}/{account}，tactics传对应的字段，然后后面再传对应的账号或者手机号。
正确示例：/base/authorization/foreign/authen/{tactics}/{其他参数}  tactics为：{accountInfoByAccount}或{accountInfoByPhone}
```
```java
    @ApiOperation(value = "查询账号信息", notes = "前端传需要查询的账号或手机号")
    @GetMapping("/base/authorization/foreign/authen/{tactics}/{account}")
    @Deprecated
    public void getDepartmentData(@PathVariable @Valid String tactics, String account) {
        Account.getInfo(tactics, account);
    }
```
