...
| 郭阳 | 2.0.2 | 2020.04.17 | 里程碑部分增加对接工作的说明 |
---
...
- 各系统对接的情况，依据对接的类型：
    - **接入**：三方系统的页面嵌入，接口数据接入，未基于页面/接口进行业务开发，仅围绕对接行为做配合的，界定为接入开发。在当前进行的里程碑中添加Issue进行说明（若当前里程碑已关闭或进入测试阶段，则存放在下一阶段里程碑中）
    - **二次开发**：基于三方接口进行完整的功能性开发，界定为二次开发。应单独设立里程碑
